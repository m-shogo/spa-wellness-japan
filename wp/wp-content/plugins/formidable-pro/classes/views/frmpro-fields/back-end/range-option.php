...
<div class="frm_form_field <?php echo $show_jquery_placeholder ? 'frm_noallow' : ''; ?>">
	<label for="frm_range_field_<?php echo esc_attr( $field['id'] ); ?>" class="frm_help frm-mb-0 frm-field-date-or-time-enable-range" title="<?php echo esc_attr( $range_option_tooltip ); ?>" >
		<input <?php disabled( ! empty( $field['is_range_end_field'] ) || $show_jquery_placeholder ); ?> type="checkbox" id="frm_range_field_<?php echo esc_attr( $field['id'] ); ?>" name="field_options[range_field_<?php echo esc_attr( $field['id'] ); ?>]" data-range-type="<?php echo esc_attr( $field['type'] ); ?>"  data-field-id="<?php echo (int) $field['id']; ?>" value="1" <?php checked( ( ! empty( $field['range_field'] ) || ! empty( $field['is_range_end_field'] ) ) && ! $show_jquery_placeholder, 1 ); ?>/>
		<?php
		/* translators: %s: Field type */
		printf( esc_html__( '%s Range', 'formidable' ), esc_html( ucfirst( $field['type'] ) ) );
		?>
	</label>
</div>
