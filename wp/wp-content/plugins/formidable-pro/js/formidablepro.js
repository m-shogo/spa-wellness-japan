function frmProFormJS() {
	...
	frmDatepickerPro.getDefaultConfigs = function( config, input ) {
		...
		this.updateRangeFieldsOnChange = ( mode, instance, dateStr, selectedDates ) => {
			...
			const startDateField         = document.querySelector( `input[data-field-id="${ fieldId }"]` );
			const [ startDate, endDate ] = selectedDates;
			const endDateField           = 'undefined' !== typeof endDate ? document.querySelector( `input[data-range-start-field-id="${ fieldId }"]` ) : null;

			instance.element.dataset.rangeValue = dateStr;

			if ( instance.element === startDateField && null !== endDateField ) {
				instance.element.value              = flatpickr.formatDate( startDate, settings.options.fpDateFormat );
			...
			if ( instance.element === endDateField ) {
				instance.element.value              = flatpickr.formatDate( endDate, settings.options.fpDateFormat );
	 ...
	 * @returns {void}
	 */
	frmDatepickerPro.setDefaultRangeValue = function( input ) {
		if ( ! input.classList.contains( 'frm_date_range' ) ) {
			return null;
		}

		const startDate = input.dataset.fieldId ? input : document.querySelector( `input[data-field-id="${ input.dataset.rangeStartFieldId }"]` );
	...
	*/
	...
	function initializeRangeSlider( rangeInput, rangeContainer ) {
		rangeInput.dataset.isRangeSliderInitialized = true;
		...
		let initialMinValue, initialMaxValue, decimalsToUse = 0;
		...
		initialMinValue = Math.max( values[0], min );
		initialMaxValue = Math.min( Math.max( values[1], initialMinValue + minRange ), max );

		const getWidthWithHiddenParent = element => {
			if ( element.closest( '.frm_form_field' ).style.display !== 'none' ) {
